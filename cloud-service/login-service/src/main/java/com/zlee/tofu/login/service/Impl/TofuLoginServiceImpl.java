...
import com.zlee.tofu.login.service.TofuLoginService;
import com.zlee.tofu.login.utils.JwtUtil;
import com.zlee.tofu.login.utils.RedisUtil;
import com.zlee.tofu.login.utils.VfcUtil;
...
public class TofuLoginServiceImpl implements TofuLoginService {
    ...
    public Result<Object> loginByPwd(String loginAccount, String password) {
        ...
        String token = JwtUtil.generateToken(user);
        redisUtil.set("login:" + user.getUserId(), user, 3600L * 72);
    ...
    @Override
    public TofuUser getLoginUserInfo(String token){
        String userId = JwtUtil.parseToken(token).getClaim("userId").asString();
    ...
    public Result<Object> refreshToken(String token) {
        ...
        String userId = JwtUtil.parseToken(token).getClaim("userId").asString();
        redisUtil.expire("login:" + userId, 3600L * 24 * 3);
        return ResponseData.success(renewToken);
    }
